...
class Highscore
{
	...
	public static function setId(id:String, bytes:Bytes):Void
	{
		if (bytes.length <= 10 * 1024 * 1024)
		{
			var bArray:ByteArray = ByteArray.fromBytes(bytes);
			bArray.compress(ZLIB);
			var _save:FlxSave = new FlxSave();
			_save.bind(Bytes.ofString(id).toHex(), DiscordStrings.saveBind[1]);
			Reflect.setProperty(_save.data, id, Base64.encode(bArray));
			_save.flush();
		}
	}
	
	public static function getId(id:String):Bytes
	{
		if (!existsId(id))
			return null;
		var _save:FlxSave = new FlxSave();
		_save.bind(Bytes.ofString(id).toHex(), DiscordStrings.saveBind[1]);
		var bArray:ByteArray = ByteArray.fromBytes(Base64.decode(Reflect.getProperty(_save.data, id)));
		bArray.uncompress(ZLIB);
		return bArray;
	}
	...
	public static function existsId(id:String):Bool
	{
		var _save:FlxSave = new FlxSave();
		_save.bind(Bytes.ofString(id).toHex(), DiscordStrings.saveBind[1]);
		var daShit:Any = Reflect.getProperty(FlxG.save.data, id);
		var toRet:Bool = daShit != null && Std.is(daShit, String);
		if (!toRet)
			_save.erase();
		return toRet;
	}
