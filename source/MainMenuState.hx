...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if visualControls
		daMultiplier++;
		daString += "Visual Controls by luckydog7\n";
		#end
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			var uppedH:Bool = false;
			var downedH:Bool = false;
			...
			uppedH = controls.UP || _pad.buttonUp.pressed;
			downedH = controls.DOWN || _pad.buttonDown.pressed;
			#if android
			...
			if (accepted)
			{
				...
				else
				{
					...
					FlxFlicker.flicker(scrollBG2, 1.1, 0.15, false);
						...
						else
						{
							FlxFlicker.flicker(spr, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								...
								switch (daChoice)
								{
									...
									case 'credit menu':
										FlxG.switchState(new PreCreditsMenu());
