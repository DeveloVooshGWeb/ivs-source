...
class PlayState extends MusicBeatState
{
	...
	public static var showDialogue:Bool = false;

	public static var maxAddScore:Int = 300;

	var enableChrome:Bool = false;
	var chromeVal:Float = 0;
	...
	var speaker1:FlxSprite;
	var speaker2:FlxSprite;
	var crowd:FlxSprite;
	var bgbg:FlxSprite;
	...
	var letsGo:Array<Array<Dynamic>> = [];
	var letsGoMode:Bool = false;
	
	var rude:Array<Array<Dynamic>> = [];
	...
	override public function create()
	{
		...
		switch(iconRPCBefore)
		{
			...
			case 'mom-car':
				iconRPCBefore = 'mom';
			case 'kuna-dafurry-forgotherspeakers' | 'kuna-furry':
				iconRPCBefore = 'kuna';
		}
		...
		DiscordClient.changePresence(DiscordStrings.modName + " v" + CurrentVersion.get() + " " + detailsText + " " + SONG.song, daShitText , iconRPC);
		...
		emitters = new FlxCamera();
		emitters.bgColor.alpha = 0;
		...
		var virtualDialogueCam:FlxCamera = new FlxCamera();
		virtualDialogueCam.bgColor.alpha = 0;
		FlxG.cameras.add(virtualDialogueCam);
		
		#if visualControls
		if (showDialogue)
		{
			_pad2 = new FlxVirtualPad(NONE, A_X);
			_pad2.alpha = 0.75;
			add(_pad2);
			_pad2.cameras = [virtualDialogueCam];
		}
		#end
		...
		if (Highscore.getShaders())
		{
			camGame.setFilters(filters);
			noteCam.setFilters(filters);
			camHUD2.setFilters(filters);
			camcontrol.setFilters(filters);
			cutsceneCam.setFilters(filters);
			virtualDialogueCam.setFilters(filters);
			pauseCam.setFilters(filters);
			camGame.filtersEnabled = true;
			noteCam.filtersEnabled = true;
			camHUD2.filtersEnabled = true;
			camcontrol.filtersEnabled = true;
			cutsceneCam.filtersEnabled = true;
			virtualDialogueCam.filtersEnabled = true;
			pauseCam.filtersEnabled = true;
		}
		...
		for (i in 0...filterList.length)
		{
			...
			if (Highscore.getEffects().contains(kys) && Highscore.getShaders())
		...
		{
			createChrome();
			if (['ivs-week1'].contains(curStage))
			{
				filters.push(colorDistortion);
			}
		}
		//WELL SHIT
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'tutorial' | 'whale-waltz' | 'interview':
		...
		else if (SONG.song.toLowerCase() == 'whale-waltz')
		{
			var letsGoSong:SwagSong = Song.loadFromJson("letsgo", "whale-waltz");
			if (letsGoSong.notes.length > 0)
			{
				letsGo = [];
				for (shitnote in letsGoSong.notes)
				{
					for (somemoreshit in shitnote.sectionNotes)
					{
						letsGo.push(somemoreshit);
					}
				}
			}
			...
			bgbg = new FlxSprite().loadGraphic(Paths.image('ivs-bg-bg')); // now this just looks like im on crack LOL
			bgbg.antialiasing = true;
			bgbg.active = false;
			add(bgbg);
			
			var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('ivs-bg'));
			bg.antialiasing = true;
			bg.active = false;
			add(bg);
			
			var fg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('ivs-fg'));
			fg.antialiasing = true;
			fg.active = false;
			add(fg);
			
			speaker1 = new FlxSprite(76, 377);
			speaker1.frames = Paths.getSparrowAtlas('speaker');
			speaker1.animation.addByPrefix('bop', 'speaker', 15, false);
			speaker1.antialiasing = true;
			add(speaker1);
			
			speaker2 = new FlxSprite(2199, 377);
			speaker2.frames = Paths.getSparrowAtlas('speaker2');
			speaker2.animation.addByPrefix('bop', 'speaker', 15, false);
			speaker2.antialiasing = true;
			add(speaker2);
			
			crowd = new FlxSprite(0, 1512);
			crowd.frames = Paths.getSparrowAtlas('crowd');
			crowd.animation.addByPrefix('bop', 'crowd', 15, false);
			crowd.antialiasing = true;
			crowd.y -= crowd.height;
			add(crowd);
			
			var centerLights:FlxSprite = new FlxSprite().loadGraphic(Paths.image('centerlights'));
			centerLights.antialiasing = true;
			add(centerLights);
			
			var light1:FlxSprite = new FlxSprite().loadGraphic(Paths.image('light1'));
			light1.antialiasing = true;
			add(light1);
			
			var light2:FlxSprite = new FlxSprite(2732, 0).loadGraphic(Paths.image('light2'));
			light2.antialiasing = true;
			light2.x -= light2.width;
			add(light2);
		...
		else if (['tutorial', 'interview'].contains(SONG.song.toLowerCase()))
		{
			if (SONG.song.toLowerCase() == 'interview')
			{
				var rudeSong:SwagSong = Song.loadFromJson("stare", "interview");
				if (rudeSong.notes.length > 0)
				{
					rude = [];
					for (shitnote in rudeSong.notes)
					{
						for (somemoreshit in shitnote.sectionNotes)
						{
							rude.push(somemoreshit);
						}
					}
				}
			}
			...
			defaultCamZoom = 0.7;
			curStage = 'ivs-tutorial';
			var mirror:FlxSprite = new FlxSprite().loadGraphic(Paths.image('tutorial/uhhmirroriguesslol'));
			mirror.antialiasing = true;
			add(mirror);
			
			tutbg = new FlxSprite().loadGraphic(Paths.image('tutorial/W A L L S ok i should stop with this stupid crap'));
			tutbg.antialiasing = true;
			add(tutbg);
			
			var FLOOR:FlxSprite = new FlxSprite().loadGraphic(Paths.image('tutorial/F L O O R')); // F L O O R make this a meme lol
			FLOOR.antialiasing = true;
			add(FLOOR);
			
			if (SONG.song.toLowerCase() == 'tutorial')
			{
				boombox = new FlxSprite(951, 420);
				boombox.frames = Paths.getSparrowAtlas('tutorial/boombox');
				boombox.animation.addByPrefix('bop', 'boombox', 24, false);
				boombox.antialiasing = true;
				add(boombox);
			}
			
			var CHAIR:FlxSprite = new FlxSprite().loadGraphic(Paths.image('tutorial/C H A I R'));
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				gfVersion = 'gf-pixel';
			case 'ivs-week1':
				gfVersion = 'kuna-furry';
			case 'ivs-tutorial':
				gfVersion = 'kuna-dafurry-forgotherspeakers';
		}
		...
		switch (SONG.player2)
		{
			...
			case 'kuna-dafurry-forgotherspeakers':
				dad.setPosition(gf.x, gf.y);
				gf.visible = false;
			...
			case 'bao':
				dad.x = 504;
				dad.y = 430;
				camPos.set(dad.getGraphicMidpoint().x + 500, dad.getGraphicMidpoint().y);
		}
		...
		switch (curStage)
		{
			...
			case 'ivs-tutorial':
				...
				if (SONG.song.toLowerCase() == 'tutorial')
				{
					...
					sing1 = new Alphabet(0, boyfriend.y + boyfriend.height - 30, "SINGING1", false, false, true, 1.1, 1.2);
					sing1.x = boyfriend.getMidpoint().x - sing1.width / 2;
					add(sing1);
					arr1 = new FlxSprite(sing1.x + sing1.width, sing1.y);
					arr1.frames = Paths.getSparrowAtlas('NOTE_assets');
					arr1.animation.addByIndices('1', 'down press', [3], "", 0, true);
					arr1.animation.addByIndices('0', 'left press', [3], "", 0, true);
					arr1.animation.addByIndices('3', 'right press', [3], "", 0, true);
					arr1.animation.addByIndices('2', 'up press', [3], "", 0, true);
					var daSize:Int = Std.int(sing1.height);
					arr1.x += (arr1.width - daSize) / 4;
					arr1.setGraphicSize(daSize, daSize);
					arr1.updateHitbox();
					add(arr1);
					sing2 = new Alphabet(0, dad.y + dad.height + 30, "SINGING2", false, false, true, 1.1, 1.2);
					sing2.x = dad.getMidpoint().x - sing2.width / 2;
					add(sing2);
					arr2 = new FlxSprite(sing2.x + sing2.width, sing2.y);
					arr2.frames = Paths.getSparrowAtlas('NOTE_assets');
					arr2.animation.addByIndices('1', 'down press', [3], "", 0, true);
					arr2.animation.addByIndices('0', 'left press', [3], "", 0, true);
					arr2.animation.addByIndices('3', 'right press', [3], "", 0, true);
					arr2.animation.addByIndices('2', 'up press', [3], "", 0, true);
					daSize = Std.int(sing2.height);
					arr2.x += (arr2.width - daSize) / 4;
					arr2.setGraphicSize(daSize, daSize);
					arr2.updateHitbox();
					add(arr2);
					sing1.visible = false;
					arr1.visible = false;
				...
				else if (SONG.song.toLowerCase() == 'interview')
				{
					dad.x = 650;
					dad.y = 850;
					/*gf.x = tutbg.width / 2 - gf.width / 2 - 40;
					gf.y = 320;*/
			...
			case 'ivs-week1':
				boyfriend.x = 1692;
				boyfriend.y = 448;
				gf.x = bgbg.width / 2 - gf.width / 2 - 40;
				gf.y = 290;
		}
		...
		doof.finishThing = finishDialogue;
		...
		scoreTxt = new FlxText(0, daPosShit, 0, "", 200);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			...
			// gwebdev's voice bullshit lol
			var daVoices2:String = Paths.voices2(PlayState.SONG.song);
			if (openfl.utils.Assets.exists(daVoices2))
			{
				vocals2 = new FlxSound().loadEmbedded(daVoices2);
			}
		}
		else
			vocals = new FlxSound();
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(DiscordStrings.modName + " v" + CurrentVersion.get() + " " + detailsText + " " + SONG.song, daShitText , iconRPC, true, songLength - Conductor.songPosition);
			}
			else
			{
				DiscordClient.changePresence(DiscordStrings.modName + " v" + CurrentVersion.get() + " " + detailsText + " " + SONG.song, daShitText , iconRPC);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			#if visualControls
			doof.accepted = controls.ACCEPT || _pad2.buttonA.justPressed;
			doof.held = FlxG.keys.pressed.S || _pad2.buttonX.pressed;
			#else
			doof.accepted = controls.ACCEPT;
			doof.held = FlxG.keys.pressed.S;
			#end
		}
		
		if (rude.length > 0 && rude[0][0] < Conductor.songPosition)
		{
			dad.rudeMode = !dad.rudeMode;
			rude.shift();
		}
		
		if (letsGo.length > 0 && letsGo[0][0] < Conductor.songPosition)
		{
			if (dad.animOffsets.exists('LETSGO'))
			{
				dad.playAnim('LETSGO', true);
				letsGoMode = true;
			}
			letsGo.shift();
		}
		
		if (letsGoMode && dad.animation.curAnim != null)
			if (dad.animation.curAnim.name != 'LETSGO')
			{
				letsGoMode = false;
		...
		if (boyfriend.curCharacter == 'crystal' && boyfriend.animation.curAnim != null)
			if (boyfriend.animation.curAnim.name.endsWith('miss'))
				boyfriend.color = 0xFF9999FF;
			else
				boyfriend.color = 0xFFFFFFFF;
				
		if (Highscore.getShaders() && enableChrome)
			setChrome(chromeVal);
			
		var chromeDelayMS:Float = 25;
		var chromeMinus:Float = 1;
		
		chromeMinus /= FlxG.width;
			
		if (chromeVal > 0)
			chromeVal -= ((1 / (chromeDelayMS / 1000)) / FlxG.updateFramerate) * chromeMinus;
		...
		scoreTxt.text += " | GWebDev's Dumb Engine";
		...
		var isDead:Bool = iconP2.animation.curAnim.curFrame == 1;
		if (isDead)
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'bao':
						...
						if (letsGoMode)
						{
							...
							FlxTween.tween(FlxG.camera, {zoom: 1.3}, (Conductor.stepCrochet * 4 / 2000));
						}
					case 'kuna-dafurry-forgotherspeakers':
						camFollow.x -= 200;
						camFollow.y += 100;
				...
				if (SONG.song.toLowerCase() == 'tutorial' && curStage == 'stage')
				{
					tweenCamIn();
				}
			}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (SONG.song.toLowerCase() == 'tutorial')
					if (daNote.y < FlxG.height * 0.7 && daNote.visible)
						if (daNote.mustPress)
							isVisible1 = true;
						else
							isVisible2 = true;
				...
				/*if ((Highscore.getDownscroll() && (daNote.isSustainNote
					&& daNote.y + daNote.offset.y >= (strumLine.y + Note.swagWidth / 2) + FlxG.height - daNote.height
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		{
			case 'ivs-week1' | 'ivs-tutorial':
				coolText.x = gf.getMidpoint().x - 40;
		}
		//
		...
		score += PlayState.maxAddScore;

		var daRating:String = "sick";
		
		var goodMultiplier:Float = (!Highscore.getBetterInput() && !Highscore.getInput()) ? 0.2 : 0.35;
		...
		else if (noteDiff > Conductor.safeZoneOffset * goodMultiplier)
		{
			daRating = 'good';
			score = 200;
			notesHit += 0.95;
		...
		{
			case 'ivs-week1' | 'ivs-tutorial':
				rating.y = gf.getMidpoint().y;
				comboSpr.y = gf.getMidpoint().y;
		}
		...
		for (i in seperatedScore)
		{
			...
			{
				case 'ivs-week1' | 'ivs-tutorial':
					numScore.y = gf.getMidpoint().y + 140;
			}
	...
	private function keyShit():Void
	{
		...
		var possibleEnemyNotes:Array<Note> = [];
		
		var ignoreEnemyList:Array<Int> = [];
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			else if (!daNote.mustPress && daNote.y < FlxG.height * 0.15 && SONG.song.toLowerCase() == 'tutorial')
			{
				possibleEnemyNotes.push(daNote);
				possibleEnemyNotes.sort((a, b) -> Std.int(a.strumTime - b.strumTime));
				
				ignoreEnemyList.push(daNote.noteData);
			}
		});
						...
						for (coolNote in possibleNotes)
						{
							...
							else
							{
								...
								if (!Highscore.getInput() && !inIgnoreList)
									badNoteCheck(coolNote.noteData);
								else //if (Highscore.getBetterInput())
									missItUp();
							}
						}
					}
		...
		// copy pasted shit for tutorial LOL
		if (SONG.song.toLowerCase() == 'tutorial')
		{
			if (possibleNotes.length > 0)
			{
				...
				if (possibleNotes.length >= 2)
				{
					if (possibleNotes[0].strumTime == possibleNotes[1].strumTime)
					{
						arr1.animation.play(Std.string(daNote.noteData), true);
					}
					else if (possibleNotes[0].noteData == possibleNotes[1].noteData)
					{
						arr1.animation.play(Std.string(daNote.noteData), true);
			...
			if (possibleEnemyNotes.length > 0)
			{
				var daNote = possibleEnemyNotes[0];

				// Jump notes
				if (possibleEnemyNotes.length >= 2)
				{
					if (possibleEnemyNotes[0].strumTime == possibleEnemyNotes[1].strumTime)
					{
						arr2.animation.play(Std.string(daNote.noteData), true);
					}
					else if (possibleEnemyNotes[0].noteData == possibleEnemyNotes[1].noteData)
					{
						arr2.animation.play(Std.string(daNote.noteData), true);
					}
					else
	...
	function noteMiss(direction:Int = 1, sus:Bool = false):Void
	{
	...
	function badNoteCheck(?daNot:Int = -1, sus:Bool = false)
	{
		// just double pasting this shit cuz fuk u
		// REDO THIS SYSTEM!
		/*var upP = controls.UP_P;
		...
		if (plzListen || !Highscore.getBetterInput())
		{
		if (leftP && daNot != 0)
			noteMiss(0, sus);
	...
	function noteCheck(keyP:Bool, note:Note):Void
	{
		...
		else
		{
			...
			{
				badNoteCheck(note.noteData, note.isSustainNote);
			}
		}
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if (Highscore.getInput() && Highscore.getBetterInput())
			{
			plzListen = false;
			if (listenTimer != null)
			{
	...
	{
		super.stepHit();
		if (FlxG.sound.music.time > Conductor.songPosition + 5 || FlxG.sound.music.time < Conductor.songPosition - 5)
		{
			resyncVocals();
		}
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case "ivs-week1":
				speaker1.animation.play('bop', true);
				speaker2.animation.play('bop', true);
				crowd.animation.play('bop', true);
				chromeVal = 3;
				if (curBeat % 4 == 0)
					chromeVal = 6;
				chromeVal /= FlxG.width;
			case "ivs-tutorial":
				if (SONG.song.toLowerCase() == 'tutorial')
